...
import {
  ...
  detectConfigTargetsFor,
  guardMissingConfig,
  removeFromEslintIgnore,
  removeFromPrettierExcluded,
  removeFromTsconfigExclude
} from './services/configTargets';
...
function checkDuplicates(newPattern: string, existingLines: string[]): boolean {
  ...
  if (!normalized) {
    return true;
  } // Empty pattern considered duplicate
...
const IGNORE_MAP: Record<IgnoreKey, { file: string; label: string; contextKey: string }> = {
  ...
  stylelint: {
    ...
    label: 'Stylelint',
    contextKey: 'confignore.hasStylelint'
  },
...
async function detectCapabilities() {
  ...
  // docker
  if (
    (await vscode.workspace.findFiles(`{${patterns.docker.join(',')}}`, '**/node_modules/**', 1))
      .length > 0
  ) {
    states.docker = true;
  }
  ...
  // eslint
  if (
    (await vscode.workspace.findFiles(`{${patterns.eslint.join(',')}}`, '**/node_modules/**', 1))
      .length > 0
  ) {
    states.eslint = true;
  }
  ...
  // prettier
  if (
    (await vscode.workspace.findFiles(`{${patterns.prettier.join(',')}}`, '**/node_modules/**', 1))
      .length > 0
  ) {
  ...
  // stylelint
  if (
    (await vscode.workspace.findFiles(`{${patterns.stylelint.join(',')}}`, '**/node_modules/**', 1))
      .length > 0
  ) {
  ...
  await Promise.all(
    (Object.keys(IGNORE_MAP) as IgnoreKey[]).map((key) =>
      setContext(IGNORE_MAP[key].contextKey, !!states[key])
    )
  );

  // Also set tsconfig context
  const hasTsconfig =
    (await vscode.workspace.findFiles('**/tsconfig.json', '**/node_modules/**', 1)).length > 0;
...
function getIncludeSupportFlag(): boolean {
  return (
    vscode.workspace.getConfiguration('ignorer.features').get<boolean>('includeSupport', false) ??
    false
  );
}
...
function getConfirmMixedState(): boolean {
  return (
    vscode.workspace.getConfiguration('confignore').get<boolean>('confirmMixedState', true) ?? true
  );
}
...
async function appendUniqueLines(
  file: vscode.Uri,
  newLines: string[]
): Promise<{ changed: boolean; duplicates: string[] }> {
...
async function addToIgnore(type: IgnoreKey, arg0?: unknown, arg1?: unknown) {
  ...
  if (targets.length === 0) {
    vscode.window.showWarningMessage(
      'No target selected. Please right-click a file or folder in the Explorer.'
    );
    return;
  }
  ...
  for (const [folder, uris] of byFolder) {
    ...
    if (result.changed) {
      ...
      vscode.window.showInformationMessage(message, 'View File').then((action) => {
        if (action === 'View File') {
          vscode.workspace
            .openTextDocument(ignoreFile)
      ...
      if (result.duplicates.length > 0) {
        vscode.window.showInformationMessage(
          `All patterns already exist in ${IGNORE_MAP[type].file}`
        );
      } else {
        vscode.window.setStatusBarMessage(
          `confignore: Nothing to add to ${IGNORE_MAP[type].file}`,
          3000
        );
      }
    }
...
export function activate(context: vscode.ExtensionContext) {
      ...
      .then((action) => {
        if (action === 'Learn More') {
          vscode.env.openExternal(
            vscode.Uri.parse('https://github.com/pradeepmouli/confignore#readme')
          );
        }
      });
    ...
    try {
      ...
      if (editor) {
        ...
        if (JSON.stringify(uris) !== JSON.stringify(lastSelection)) {
          ...
          channel.appendLine(
            `[confignore] Selection state: excluded=${state.excluded}, mixed=${state.mixed}, source=${state.source}`
          );
        }
      }
    } catch (err) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('confignore.addToIgnore.tsconfig', async (arg0, arg1) => {
      ...
      if (items.length === 0) {
        vscode.window.showWarningMessage('No selection to exclude.');
        return;
      }
      const targets = await detectConfigTargetsFor(items[0]);
      if (!(await guardMissingConfig(targets?.tsconfig, 'tsconfig'))) {
        return;
      }
    ...
    ...(includeSupportEnabled
      ? [
          vscode.commands.registerCommand('confignore.include', async (arg0, arg1) => {
            ...
            const items = collectTargetUris(arg0, arg1);
            if (items.length === 0) {
              vscode.window.showWarningMessage('No selection to include.');
              return;
            }
            // For v1: attempt removal from config-based sources in precedence order
            const targets = await detectConfigTargetsFor(items[0]);
            let changed = false;
            if (!changed && targets?.tsconfig) {
              changed = await removeFromTsconfigExclude(targets.tsconfig, items);
            }
            if (!changed && targets?.eslintConfig) {
              changed = await removeFromEslintIgnore(targets.eslintConfig, items);
            }
            if (!changed && targets?.prettierConfig) {
              changed = await removeFromPrettierExcluded(targets.prettierConfig, items);
            }
            vscode.window.setStatusBarMessage(
              changed
                ? 'confignore: Inclusion updated'
                : 'confignore: Nothing to include in configs',
              3000
            );
          })
        ]
