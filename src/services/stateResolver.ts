...
export async function resolveState(
  uri: Uri,
  options?: { includeSupportEnabled?: boolean }
  ...
  const sources: Array<{ source: Source; check: () => Promise<boolean> }> = [
    ...
    {
      source: Source.ConfigTsconfig,
      check: () =>
        includeSupportEnabled
          ? checkTsconfigExclusion(workspaceFolder.uri.fsPath, relativePath, uri)
          : Promise.resolve(false)
    },
    {
      source: Source.ConfigPrettier,
      check: () =>
        includeSupportEnabled
          ? checkPrettierExclusion(workspaceFolder.uri.fsPath, relativePath, uri)
          : Promise.resolve(false)
    },
    {
      source: Source.ConfigEslint,
      check: () =>
        includeSupportEnabled
          ? checkEslintExclusion(workspaceFolder.uri.fsPath, relativePath, uri)
          : Promise.resolve(false)
    },
    ...
    // Ignore files (middle precedence)
    {
      ...
      check: () => checkIgnoreFile(workspaceFolder.uri.fsPath, '.gitignore', relativePath)
    },
    {
      source: Source.IgnoreFileDocker,
      check: () => checkIgnoreFile(workspaceFolder.uri.fsPath, '.dockerignore', relativePath)
    },
    {
      source: Source.IgnoreFileEslint,
      check: () => checkIgnoreFile(workspaceFolder.uri.fsPath, '.eslintignore', relativePath)
    },
    {
      source: Source.IgnoreFilePrettier,
      check: () => checkIgnoreFile(workspaceFolder.uri.fsPath, '.prettierignore', relativePath)
    },
    {
      source: Source.IgnoreFileNpm,
      check: () => checkIgnoreFile(workspaceFolder.uri.fsPath, '.npmignore', relativePath)
    },
    {
      source: Source.IgnoreFileStylelint,
      check: () => checkIgnoreFile(workspaceFolder.uri.fsPath, '.stylelintignore', relativePath)
    },
    {
      source: Source.IgnoreFileVscode,
      check: () => checkIgnoreFile(workspaceFolder.uri.fsPath, '.vscodeignore', relativePath)
    },
 ...
 */
async function checkTsconfigExclusion(
  workspacePath: string,
  relativePath: string,
  uri: Uri
): Promise<boolean> {
 ...
 */
async function checkPrettierExclusion(
  workspacePath: string,
  relativePath: string,
  uri: Uri
): Promise<boolean> {
  const targets = await detectConfigTargetsFor(uri);
  if (!targets?.prettierConfig) {
    return false;
  }
 ...
 */
async function checkEslintExclusion(
  workspacePath: string,
  relativePath: string,
  uri: Uri
): Promise<boolean> {
  const targets = await detectConfigTargetsFor(uri);
  if (!targets?.eslintConfig) {
    return false;
  }
